...
title_lkc = "\x1b[96m┣━▶ Generating LKC for node %d"+"\x1b[0m"
lkc_done = "\x1b[96m┣━▶ \x1b[0m%s"

title_tv = "\x1b[95m┣━▶ Generating TV for %d ▬▶ %d"+"\x1b[0m"
tv_done = "\x1b[95m┣━▶ \x1b[0m%s"

title_tkc = "\x1b[93m┣━▶ Generating TKC for %d ▬▶ %d"+"\x1b[0m"
tkc_done = "\x1b[93m┣━▶ \x1b[0m%s"

title_ss = "\x1b[94m┣━▶ TAK for %d ▬▶ %d"+"\x1b[0m"
ss_done = "\x1b[94m┣━▶ \x1b[0m%s"
...
def main(args):
	...
	for n in range(node_number):
		...
		print(lkc_done % export_to_hexstring(lkc, False))
	"""
	TV
	"""
	for i in range(node_number):
		...
		for j in all_but_i:
			...
			print(tv_done % export_to_hexstring(tv, False))
	"""
	TAK
	"""
	for i in range(node_number):
	...
	for i in range(node_number):
		...
		for j in all_but_i:
			...
			print(tkc_done % export_to_hexstring(tkc, False))
	for i in range(node_number):
		all_but_i = list(range(node_number))
		all_but_i.remove(i)
		for j in all_but_i:
			print(title_ss % (i, j))
			print(ss_done % key_to_hexstring(nodes[i]["ss"][j]))
	return 0
